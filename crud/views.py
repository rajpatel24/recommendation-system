...
from .models import Member, Document, Ajax, CsvUpload, Product, Order, Whishlist
...
@login_required
def index(request):
    # memory_user_item_based_filter
    id_list_to_exclud = []
    wishlist_products = Whishlist.objects.filter(user=request.user).values_list('product_id', flat=True)
    test1 = [id_list_to_exclud.append(_id) for _id in wishlist_products]
    wishlist_query = Product.objects.filter(id__in=id_list_to_exclud)

    id_list_to_exclud2 = []
    order_products = Order.objects.filter(user=request.user).values_list('product_id', flat=True)
    test2 = [id_list_to_exclud2.append(_id) for _id in order_products]
    order_query = Product.objects.filter(id__in=id_list_to_exclud2)

    new_list = id_list_to_exclud + id_list_to_exclud2

    product_query = Product.objects.exclude(id__in=new_list)

    combine_query  = wishlist_query.union(order_query)

    #Content based filter model
    category = "Computers&Accessories|Accessories&Peripherals|Cables&Accessories|Cables|USBCables"
    content_based_filter = Product.objects.filter(category__iexact=category).order_by('-rating')[:5]

    return render(request, 'index.html', {'combine_query' : combine_query, 'memory_user_item_based_filter': product_query, 'content_based_filter': content_based_filter})
...
def list(request):
    ...
    return render(request, 'list.html', {
        'products1': products,
        'products': products,
        'rating_wise_recommended_products': rating_wise_recommended_products
    })
...
def buy_product(request, product_id):
    ...
    order = Order.objects.create(product=product, user_id=request.user.id)
...
def add_to_wishlist(request, product_id):
    product = Product.objects.get(id=product_id)
    order = Whishlist.objects.create(product=product, user_id=request.user.id)
    messages.success(request, 'Added to Wishlist!')
    return redirect('list')    

@login_required
def wishlist(request):
    wishlist = Whishlist.objects.select_related('product').all()
    products = [ws.product for ws in wishlist]
...
def order(request):
    ...
    return render(request, 'order-list.html', {'products': products})

# @login_required
def rank_based_data(request):
    products = Product.objects.all()
    products_json = serialize('json', products)
    products_data = json.loads(products_json)
    sorted_items = sorted(products_data, key=lambda x: x['fields']['totalReviews'], reverse=True)
    top_n_items = sorted_items[:13]

    key_list = []
    value_list = []

    for item in top_n_items:
        key_list.append(item['fields']['title'])
        value_list.append(item['fields']['totalReviews'])

    final_data = { 'key_list': key_list, 'value_list': value_list }
